...
import type { Vehicle, VehicleStatus } from "@logistics/shared";
...
export default function VehicleEditDrawer({ vehicle, onClose, onSaved }: Props) {
  ...
  const [capacityLiters, setCapacityLiters] = useState(vehicle.capacityLiters?.toString() ?? "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const updated = await api.put<Vehicle>(`/vehicles/${vehicle.id}`, {
        licensePlate,
        type,
        status,
        brand: brand || null,
      ...
      <div className="modal-window" style={{
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ color: "#64748b", fontSize: 13, textTransform: "capitalize" }}>Edit Vehicle Info</div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: "auto", padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: "auto" }}>
            ...
            <button type="submit" disabled={submitting} style={{ flex: 1, padding: "10px 0", borderRadius: 6, border: "none", background: submitting ? "#D6D3D1" : "#D97757", color: "#fff", fontWeight: 600, fontSize: 14, cursor: submitting ? "not-allowed" : "pointer" }}>
