...
export default function AddVehicleModal({ onClose, onAdded }: Props) {
  ...
  const input: React.CSSProperties = { padding: "8px 12px", borderRadius: 6, border: "1px solid var(--border)", fontSize: 14, background: "var(--panel)", color: "var(--text)" };
  ...
  return (
    <>
      ...
      <div className="modal-window" style={{
        ...
        background: "var(--panel)", zIndex: 101, boxShadow: "var(--shadow-modal)",
        display: "flex", flexDirection: "column",
      }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontWeight: 700, fontSize: 18, color: "var(--text)" }}>Add Vehicle</div>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: "auto", padding: 24, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          {error && <span style={{ color: "var(--danger-soft-text)", fontSize: 13 }}>{error}</span>}

          <div style={{ display: "flex", gap: 10, marginTop: "auto" }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: "10px 0", borderRadius: 6, border: "1px solid var(--border)", background: "var(--panel)", color: "var(--text)", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
              Cancel
            </button>
            <button type="submit" disabled={submitting || !licensePlate.trim()} style={{ flex: 1, padding: "10px 0", borderRadius: 6, border: "none", background: submitting || !licensePlate.trim() ? "var(--border-strong)" : "#D97757", color: "#fff", fontWeight: 600, fontSize: 14, cursor: submitting || !licensePlate.trim() ? "not-allowed" : "pointer" }}>
